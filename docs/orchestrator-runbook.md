...
- import/export and review/apply:
  ...
  - closeout completion lifecycle is centralized into one shared completion-thread transition summary that records:
    ...
    - lifecycle reasons and the recommended next operator step
  - closeout completion decision audit is centralized into one shared completion-decision snapshot trail that records:
    - the latest completion action plus the exact disposition/lifecycle/carry-forward/resolution snapshots used for that decision
    - whether the decision left the thread finalized, retained, or reopened
    - queue-retained reasons plus missing follow-up or missing-evidence signals when the thread still cannot leave governance
  - closeout completion decision history is centralized into one shared completion-thread history that records:
    - repeated `confirm_review_complete`, `confirm_closeout_complete`, `keep_carry_forward`, and `reopen_completion` patterns
    - repeated queue-retained patterns plus repeated finalized-to-reopened patterns
    - the latest and previous completion decision audits so operators can see how the thread evolved
  - closeout completion finalization summary is centralized into one shared finalization decision that records:
    - whether the thread is `final_complete`, `finalized_but_reopenable`, `retained`, `reopened`, `followup_open`, or `queue_retained`
    - whether the case may actually be treated as review-complete, closeout-complete, or final-complete
    - unresolved finalization reasons and the recommended next operator step
  ...
  - review-thread interpretation rules:
    ...
    - `sandbox:closeout:completion:decision:audit`, `sandbox:closeout:completion:decision:history`, and `sandbox:closeout:completion:finalization:summary` should be read together when you need to know why a completion decision was taken, whether the thread repeatedly retained or reopened, and whether finalized still falls short of true final-complete
    - repeated `keep_carry_forward` or repeated `reopen_completion` means the completion thread is not final-complete even if one individual action was accepted
    - finalized does not automatically mean final-complete; if decision history shows finalized-to-reopened churn or finalization summary still says `followup_open` / `queue_retained`, the case must remain in governance
    - `sandbox:closeout:finalization:history`, `sandbox:closeout:finalization:stability`, and `sandbox:closeout:post-finalization:followup:queue` should be read together when you need to know whether a once-finalized thread later reopened, whether post-finalization follow-up still blocks stability, and which cases must remain in governance after they already reached final-complete once
    - `sandbox:closeout:stability:drift`, `sandbox:closeout:reopen:recurrence`, and `sandbox:closeout:stability:watchlist` should be read together when you need to know whether a once-stable thread has started drifting, whether reopen has become a recurring pattern, and which cases must stay under stability observation even after they already reached finalization
    - `sandbox:closeout:stability:recurrence:audit`, `sandbox:closeout:watchlist:resolution`, and `sandbox:closeout:watchlist:lifecycle` should be read together when you need to know whether drift/reopen/watchlist risk is repeating, whether a watchlist case can actually leave governance, and whether the current watchlist state is active, retained, resolved, or re-added
    - `sandbox:closeout:watchlist:exit:audit`, `sandbox:closeout:watchlist:readd:history`, and `sandbox:closeout:stability:recovery:summary` should be read together when you need to know why a watchlist case was allowed to leave, why it was later re-added, and whether the thread is truly recovered or only provisionally calmer
    - `sandbox:closeout:recovery:confidence:trend`, `sandbox:closeout:regression:resolution`, and `sandbox:closeout:recovered:monitoring:exit:audit` should be read together when you need to know whether recovered confidence is improving or degrading, whether regression has actually cleared, and whether monitoring can truly be exited
    - `sandbox:closeout:recovery:clearance:audit`, `sandbox:closeout:recovered:exit:history`, and `sandbox:closeout:recovered:lifecycle` should be read together when you need to know whether a recovered case can fully leave recovery governance, whether it later re-entered after exit, and whether the current lifecycle is monitored, cleared, re-entered, or regressed
    - finalized after one decision still does not equal `stable-final-complete`; if finalization history shows `finalized then reopened`, `finalized but follow-up remained open`, or `finalized then retained again`, the thread must not be treated as stably complete
    - post-finalization follow-up queue must remain open whenever a case reached final-complete once but later re-entered governance because of reopen, retained carry-forward, or post-finalization follow-up evidence gaps
    - `stability drift` means a thread that once looked final or stable later degraded because of reopen, renewed follow-up, or carry-forward retention; drift reasons should be treated as the authoritative explanation for why the thread is no longer stable
    - repeated `reopen_completion`, repeated `finalized then reopened`, or repeated retained-after-reopen in reopen recurrence means the case should not be treated as stably complete, even if one later action briefly re-finalized the thread
    - stability watchlist must remain open whenever drift risk, reopen recurrence, post-finalization follow-up, or retained queue risk remains; a case should only leave the watchlist when the watchlist is empty and finalization stability still says `stable-final-complete`
    - repeated drift, repeated reopen-after-finalization, or repeated watchlist re-add in stability recurrence audit means the risk pattern is accumulating and the case should not be treated as finally observed-away
    - watchlist resolution may only say `watchlist_resolved` when drift, reopen recurrence, follow-up, retained queue risk, and terminal severity blockers are all absent; otherwise the watchlist must stay retained or be marked re-added
    - watchlist lifecycle should be treated as the canonical operator-facing state transition summary: `active`, `retained`, `resolved`, and `re_added` are governance outcomes, not execution workflow states
    - watchlist exit audit may say `exit_allowed_but_reopenable`; that means governance can remove the case from the watchlist for now, but the thread still carries reopen risk and must not be described as fully recovered without the recovery summary agreeing
    - watchlist re-add history is the canonical explanation for `resolved then re-added`, `exit then drift`, `exit then follow-up open`, and `exit then reopen`; if re-add history remains active, the case should not be treated as genuinely recovered
    - stability recovery summary may only say `recovery_achieved` when drift, reopen recurrence, post-finalization follow-up, watchlist openness, and re-add risk have all cleared; if recovery is only `provisional`, the case still needs observation
    - recovery confidence is the canonical operator read for `high_confidence_recovered`, `provisional_recovered`, `low_confidence_recovered`, and `recovery_blocked`; if a case is recovered but still reopenable or recently cleared from watchlist governance, confidence must stay below high
    - recovery regression audit is the canonical explanation for `recovered then drifted`, `recovered then reopened`, `recovered then follow-up reopened`, and `recovered then watchlist re-added`; any active regression means the case should not be treated as currently recovered
    - recovered monitoring queue is the canonical observation backlog after recovery; if recovery is provisional, low-confidence, reopenable, recently exited from the watchlist, or carries regression risk, the case must stay on monitoring instead of being described as fully out of governance
    - recovery confidence trend is the canonical explanation for `improving_to_high_confidence`, `stable_but_provisional`, `degrading_to_low_confidence`, and `oscillating_confidence`; repeated provisional or low-confidence recovery means the case has not earned a fully trusted recovered interpretation
    - regression resolution summary may only say `regression_resolved` when drift, reopen, follow-up reopen, watchlist re-add, and terminal severity blockers have all cleared; if it is only `regression_provisionally_resolved`, the case still should not leave monitoring
    - recovered monitoring exit audit may only allow queue exit when confidence is high, regression resolution is fully resolved, monitoring is otherwise empty, and reopenable risk has cleared; if a case is recovered but still reopenable or only provisionally resolved, monitoring must stay in place
    - recovery clearance audit may say `clearance_allowed_but_reopenable`; that means monitoring may be quiet enough to consider clearance, but reopenable risk still prevents the case from being described as fully cleared from governance
    - `sandbox:closeout:recovery:clearance:history` is the canonical record of repeated `clearance_allowed`, repeated blocked clearance, `cleared then re-entered`, and `cleared then regressed`; if the same case clears repeatedly and then comes back, treat the history as governance churn rather than clean closure
    - recovered exit history is the canonical explanation for `exit then re-enter`, `cleared then reopened`, `cleared then follow-up reopened`, and `cleared then watchlist re-added`; repeated re-entry means the case should be treated as recovery-governance churn rather than fully clear
    - recovered lifecycle is the canonical operator-facing state transition summary after recovery: `recovered_monitored`, `recovered_cleared`, `recovered_reentered`, `recovered_regressed`, and `recovered_but_reopenable` are governance outcomes, not execution workflow states
    - `sandbox:closeout:recovered:reentry:audit` is the canonical explanation for `monitoring_exit_then_reenter`, `cleared_then_reenter`, `recovered_then_regressed`, and `reopened_after_cleared`; if a cleared case re-enters, treat that as a governance regression unless evidence shows the previous clearance was never actually final
    - `sandbox:closeout:recovered:lifecycle:history` is the canonical long-running transition log for `monitored -> cleared`, `cleared -> re-entered`, `re-entered -> monitored`, and `recovered -> regressed`; repeated transitions mean the case is still unstable even if the latest snapshot looks temporarily quiet
    - `sandbox:closeout:recovery:retirement:audit`, `sandbox:closeout:recovered:retirement:summary`, and `sandbox:closeout:recovery:retirement:queue` should be read together when you need to know whether a recovered case can leave active governance, why retirement is still blocked, and which recovered cases must remain in the active governance backlog
    - recovery retirement audit may say `retirement_allowed_but_reopenable`; that means the case is close to leaving active governance, but reopenable or churn risk still prevents it from being treated as fully retired
    - recovered retirement summary is the canonical operator read for `retirement_ready`, `retirement_provisional`, `retirement_blocked`, `recovered_but_still_active`, and `recovered_and_retireable`; if re-entry, regression, follow-up, or reopenable risk remains, the case must stay active
    - recovery retirement queue is the canonical backlog after recovery clearance; if a recovered case still carries regression risk, re-entry churn, or monitoring obligations, it must remain in the active governance backlog instead of disappearing from operator view
    - `sandbox:closeout:recovery:retirement:history`, `sandbox:closeout:retirement:exit:criteria`, and `sandbox:closeout:retired-case:audit:history` should be read together when you need to know how retirement decisions accumulated over time, whether the current case actually satisfies exit criteria, and whether a retired case later re-entered governance, regressed, or was re-added to watchlist observation
    - retirement exit criteria may report `provisional_pass`; that means the case is close to retirement and may already look recovered, but unresolved reopenable, re-entry, follow-up, or trend risk still prevents it from being treated as a strict retirement pass
    - a retired case that later shows re-entry, regression, or watchlist re-add in retired-case audit history must be treated as re-entering governance, even if one earlier retirement audit or summary briefly said the case was retireable
    - recovery retirement history is the canonical record of repeated retired, repeated blocked, retired-then-reentered, retired-then-regressed, and retired-then-watchlist-readded churn; if the same case keeps retiring and returning, treat it as governance churn rather than stable retirement
