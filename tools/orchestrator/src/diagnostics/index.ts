...
export type OrchestratorDiagnostics = {
  ...
  externalAutomation: {
    ...
    sourceLaneClassification: string | null;
    automaticTriggerStatus: OrchestratorState["lastGptCodeAutomationState"] extends null ? never : string;
    targetAdapterStatus: OrchestratorState["lastGptCodeAutomationState"] extends null ? never : string;
    targetLaneClassification: string | null;
    ...
    dispatchReliabilityOutcome: string;
    externalAutomationOutcome: string;
    dispatchHistorySummary: string | null;
    ...
    recoveryQueueClassification: string;
    replayRecommendation: string | null;
    resumeRecommendation: string | null;
    operatorActionRecommendation: string | null;
    recentRecoveryHistory: string[];
    repeatedFailurePattern: string | null;
    recoveryAuditSummary: string | null;
...
function resolveNextSuggestedAction(state: OrchestratorState, preflight: PreflightResult | null) {
  if (state.lastGptCodeAutomationState?.operatorActionRecommendation) {
    return state.lastGptCodeAutomationState.operatorActionRecommendation;
  }
  ...
  if (state.lastGptCodeAutomationState?.recommendedNextStep) {
    return state.lastGptCodeAutomationState.recommendedNextStep;
  }
...
export function buildDiagnosticsSummary(state: OrchestratorState, preflight: PreflightResult | null = state.lastPreflightResult) {
  ...
  return {
    ...
    externalAutomation: {
      sourceType: state.lastGptCodeAutomationState?.sourceType ?? null,
      sourceLaneClassification: state.lastGptCodeAutomationState?.sourceLaneClassification ?? null,
      automaticTriggerStatus: state.lastGptCodeAutomationState?.automaticTriggerStatus ?? "not_triggered",
      targetAdapterStatus: state.lastGptCodeAutomationState?.targetAdapterStatus ?? "not_attempted",
      targetLaneClassification: state.lastGptCodeAutomationState?.targetLaneClassification ?? null,
      targetDestination: state.lastGptCodeAutomationState?.targetDestination ?? null,
      routingDecision: state.lastGptCodeAutomationState?.routingDecision ?? null,
      fallbackDecision: state.lastGptCodeAutomationState?.fallbackDecision ?? null,
      attemptCount: state.lastGptCodeAutomationState?.targetAttemptCount ?? 0,
      retryCount: state.lastGptCodeAutomationState?.targetRetryCount ?? 0,
      maxAttempts: state.lastGptCodeAutomationState?.targetMaxAttempts ?? 0,
      canRetryDispatch: state.lastGptCodeAutomationState?.canRetryDispatch ?? false,
      dispatchExhausted: state.lastGptCodeAutomationState?.dispatchExhausted ?? false,
      lastFailureClass: state.lastGptCodeAutomationState?.lastTargetFailureClass ?? null,
      dispatchReliabilityOutcome: state.lastGptCodeAutomationState?.dispatchReliabilityOutcome ?? "not_run",
      externalAutomationOutcome: state.lastGptCodeAutomationState?.externalAutomationOutcome ?? "not_run",
      ...
      recoveryQueueClassification: state.lastGptCodeAutomationState?.recoveryQueueClassification ?? "not_applicable",
      replayRecommendation: state.lastGptCodeAutomationState?.replayRecommendation ?? null,
      resumeRecommendation: state.lastGptCodeAutomationState?.resumeRecommendation ?? null,
      operatorActionRecommendation: state.lastGptCodeAutomationState?.operatorActionRecommendation ?? null,
      recentRecoveryHistory: state.lastGptCodeAutomationState?.recentRecoveryHistory ?? [],
      repeatedFailurePattern: state.lastGptCodeAutomationState?.repeatedFailurePattern ?? null,
      recoveryAuditSummary: state.lastGptCodeAutomationState?.recoveryAuditSummary ?? null,
...
export function formatDiagnosticsSummary(summary: OrchestratorDiagnostics) {
  ...
  if (summary.externalAutomation.dispatchHistorySummary) {
    lines.push(`External automation dispatch history: ${summary.externalAutomation.dispatchHistorySummary}`);
  }
  if (summary.externalAutomation.fallbackChainSummary) {
    lines.push(`External automation fallback summary: ${summary.externalAutomation.fallbackChainSummary}`);
  }
  if (summary.externalAutomation.recoverabilitySummary) {
    lines.push(`External automation recoverability: ${summary.externalAutomation.recoverabilitySummary}`);
  }
  if (summary.externalAutomation.recoveryAuditSummary) {
    lines.push(`External automation recovery audit: ${summary.externalAutomation.recoveryAuditSummary}`);
  }
  ...
  if (summary.externalAutomation.replayRecommendation) {
    lines.push(`External automation replay recommendation: ${summary.externalAutomation.replayRecommendation}`);
  }
  if (summary.externalAutomation.resumeRecommendation) {
    lines.push(`External automation resume recommendation: ${summary.externalAutomation.resumeRecommendation}`);
  }
  if (summary.externalAutomation.operatorActionRecommendation) {
    lines.push(`External automation operator action: ${summary.externalAutomation.operatorActionRecommendation}`);
  }
  ...
  if (summary.externalAutomation.repeatedFailurePattern) {
    lines.push(`External automation repeated failure pattern: ${summary.externalAutomation.repeatedFailurePattern}`);
  }
  if (summary.externalAutomation.recentRecoveryHistory.length > 0) {
    lines.push("External automation recent recovery history:");
    for (const item of summary.externalAutomation.recentRecoveryHistory) {
      lines.push(`- ${item}`);
    }
  }
