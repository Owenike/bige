...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (upload.error) {
    return NextResponse.json({ ok: false, error: "本人照片上傳失敗，請重新拍攝。" }, { status: 500 });
  }
  ...
  if (saved.error || !saved.data) {
    ...
    return NextResponse.json({ ok: false, error: "本人照片已存在；請重新整理後繼續。" }, { status: 409 });
  }

  const signed = await admin.storage.from(STUDENT_PHOTO_BUCKET).createSignedUrl(photoPath, 10 * 60);
  if (signed.error || !signed.data?.signedUrl) {
    return NextResponse.json({ ok: false, error: "本人照片已儲存，請重新整理後繼續。" }, { status: 500 });
  }
