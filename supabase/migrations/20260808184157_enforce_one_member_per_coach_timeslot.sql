...
-- "Unlimited weight training" means the tenant may schedule any number of
-- coaches for weight training at the same time. Each individual coach still
-- owns a single schedule range and can serve only one member in that range.
-- The lock is scoped by tenant, coach, and business date so concurrent writes
-- for different coaches do not block one another.
