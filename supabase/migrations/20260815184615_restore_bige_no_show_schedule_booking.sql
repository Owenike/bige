...
-- Restore a schedule entry that was marked no-show without consuming or
-- returning any contract sessions. Restoring is an administrative correction,
-- so authorized schedule managers may do it at any time; active-schedule
-- conflict checks still protect the member, coach, and shared classrooms.
create or replace function public.bige_restore_no_show_schedule_booking(
  p_booking_id uuid
)
returns jsonb
language plpgsql
...
begin
  ...
  if booking_row.status <> 'no_show' then
    raise exception 'no_show_restore_not_available';
  end if;
  ...
  set status = 'booked',
      ...
      status_reason = 'no_show_reversed',
      updated_at = now()
  ...
  set status = 'reopened',
      ...
      reopened_at = now(),
      reopen_reason = 'booking_no_show_reversed_after_confirmation',
      confirmed_by = null,
      confirmed_at = null,
      updated_at = now()
  ...
  insert into public.audit_logs (
    ...
    actor.id,
    'fitness_session_no_show_reversed',
    'booking',
    booking_row.id::text,
    'no_show_reversed',
    jsonb_build_object(
      ...
      'operationKind', booking_row.operation_kind,
...
revoke all on function public.bige_restore_no_show_schedule_booking(uuid)
  from public, anon;
grant execute on function public.bige_restore_no_show_schedule_booking(uuid)
