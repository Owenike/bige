...
as $function$
begin
  new.before_state := public.redact_system_audit_json(new.before_state);
  new.after_state := public.redact_system_audit_json(new.after_state);
  new.metadata := public.redact_system_audit_json(new.metadata);
  return new;
end;
$function$;
...
drop trigger if exists sanitize_system_audit_event_before_write
  on public.system_audit_events;
create trigger sanitize_system_audit_event_before_write
